...
class PhotoManager:
    ...
    def __init__(self, config_path="config/config.json", display_instance=None):
        ...
        self.viewed_photos_file = os.path.join(
            self.config["photos"].get("directory", "static/images/photos"),
            ".viewed_photos.json",
        )
    ...
    def get_random_photo(self):
        ...
        for photo_path in photos:
            ...
            if (
                photo_path in self.photo_history[-min(3, len(photos) // 4) :]
            ):  # Avoid very recent photos
                continue
            ...
            if photo_basename in self.viewed_photos:
                ...
                weight -= min(
                    50, view_count * 5
                )  # Reduce by 5 for each past view, max 50 reduction
        ...
        if not weights:
            ...
            selected_photo = random.choices(
                photos_list, weights=normalized_weights, k=1
            )[0]
        ...
        self.viewed_photos[photo_basename]["count"] = (
            self.viewed_photos[photo_basename].get("count", 0) + 1
        )
    ...
    def create_status_bar(self, width, height):
        ...
        if weather_data:
            ...
            text_x = (width - text_width) // 2
            draw.text(
                (text_x, 11), no_weather_text, font=self.font_small, fill=fg_color
            )
    ...
    def create_clock_display(self):
        ...
        time_y = (
            display_height - time_height
        ) // 2 - 60  # Shift up to make room for date
        ...
        if weather_data:
            ...
            draw.text(
                (weather_x, weather_y), weather_text, font=self.font_bold, fill=fg_color
            )
    ...
    def create_info_display(self):
        ...
        total_photos = len(
            [
                f
                for f in os.listdir(self.config["photos"]["directory"])
        ...
        display_mode = (
            "Color"
            if self.display.is_color_display
            else ("Grayscale" if self.display.grayscale_mode else "Black & White")
        )
        dithering = (
            "Enabled"
            if self.config["display"].get("enable_dithering", True)
            else "Disabled"
        )
        ...
        if weather_data:
            draw.line(
                [(50, y_position), (display_width - 50, y_position)],
                fill=fg_color,
                width=1,
            )
            y_position += 30
            ...
            draw.text(
                (
                    (display_width - self.font_bold.getbbox(weather_title)[2]) // 2,
                    y_position,
                ),
                weather_title,
                font=self.font_bold,
    ...
    def _display_error_message(self, message):
        ...
        try:
            ...
            draw.text(
                (msg_x, display_height // 2), message, font=self.font, fill=fg_color
            )
            ...
            self.display.display_image_buffer(
                canvas, True
            )  # Force full refresh for errors
    ...
    def run_photo_cycle(self):
        ...
        try:
            ...
            while True:
                ...
                if (now - self.last_photo_change >= twelve_hours) or (
                    now % (10 * interval_seconds) < interval_seconds
                ):
                    force_refresh = True
                ...
                check_interval = min(
                    60, interval_seconds // 4
                )  # Check at least every minute, or more often for short intervals
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--test-all", action="store_true", help="Test all display modes"
    )
    ...
    try:
        if args.test_all:
            ...
            if os.path.exists(args.photo):
                ...
                manager._display_error_message(
                    f"Photo not found: {os.path.basename(args.photo)}"
                )
        else:
